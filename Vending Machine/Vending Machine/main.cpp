...
void purchaseItem(Product& item, int& currentBalance) { // Function which is used when user selects an item to purchase
    int itemCost;
...
int enterMoney(int currentBalance) { // Simple function for topping up balance by taking the balances current value and adding on the entered amount of new balance
    int newMoney;
...
void foodMenu(Product& food1, Product& food2, Product& food3, Product& food4, Product& food5, int& currentBalance) { // Main menu for the purchasing of food.
    int menuChoice;
    char continueShopping;

    do { // Food menu is printed with the displayMenu function being used to print out the item name and stock for each item after its corresponding number
        cout << "Food Menu:" << endl;
        cout << "1. ";
        displayMenu(food1);
        cout << "2. ";
        ...
        switch (menuChoice) { // Depending on what item the user selects, the purchaseItem function is run with the item of choice and the users current balance
        case 1:
        ...
        if (currentBalance >= 0) { // through the use of a while loop,if  the users balance is 0 or above, the user will be asked if they wish to continue shopping
...
void drinkMenu(Product& drink1, Product& drink2, Product& drink3, Product& drink4, Product& drink5, int& currentBalance) {
    ...
    do { //Just like the food menu, the drinks menu is printed with the displayMenu function being used to print out the item name and stock for each item after its corresponding number
...
void mainMenu(Product &food1, Product &food2, Product &food3, Product &food4, Product &food5, Product &drink1, Product &drink2, Product &drink3, Product &drink4, Product &drink5, int &currentBalance){
 ...
 int main() { // Objects are created for each of the food and drink items in the vending machine, an initial user balance is created, the menu function is ran with all of the new values being passed in
     ...
     mainMenu(drink1, drink2, drink3, drink4, drink5, food1, food2, food3, food4, food5, currentBalance);
